...
async function bootDashboard() {
  ...
  startPolling();
  if (!location.hash) {
    // Real bug: this referenced a `ROUTES` map that stopped existing when the
    // eleven sidebar panels were consolidated into five `PAGES`. On a first
    // visit (no hash in the URL) the ReferenceError escaped bootDashboard, so
    // `router()` below never ran and the dashboard rendered nothing at all.
    // currentRoute() already sanitizes an unknown/stale saved route back to
    // DEFAULT_ROUTE, so no validation is needed here beyond a fallback.
    location.hash = "#/" + (lsGet(LS_KEY_ROUTE) || DEFAULT_ROUTE);
  }
  router();
